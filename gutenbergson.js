...
var render = (()=>{return ((canvas)=>{return ((ctx)=>{return (()=>{canvas.width=$Mn(window.innerWidth,20);canvas.height=$Mn(window.innerHeight,20);ctx.font="26px californian";return ((base)=>{for$Mneach(((line)=>{ctx.fillText(line,left$Mnmargin,$Mn(base,vertical$Mnscroll));return base=$Pl(base,line$Mnheight);}),list$Mnref(chapters,current$Mnchapter));ctx.fillText("🠞",$St(4,left$Mnmargin),$Mn(base,vertical$Mnscroll));nextarrow$Mnleft=$St(4,left$Mnmargin);nextarrow$Mntop=$Mn(base,vertical$Mnscroll);((arrow)=>{nextarrow$Mnwidth=arrow.width;return nextarrow$Mnheight=arrow.emHeightAscent;})(ctx.measureText("🠞"));return content$Mnheight=$Pl(base,line$Mnheight,top$Mnmargin);})(top$Mnmargin);})();})(canvas.getContext("2d"));})(document.getElementById("canvas"));});
window.addEventListener("resize",render);
window.addEventListener("load",render);
window.addEventListener("click",((event)=>{return (((($Ls$Eq(nextarrow$Mnleft,event.x,$Pl(nextarrow$Mnleft,nextarrow$Mnwidth)))===false?(false):((($Ls$Eq(nextarrow$Mntop,event.y,$Pl(nextarrow$Mntop,nextarrow$Mnheight)))===false?(false):($Ls($Pl(current$Mnchapter,1),length(chapters)))))))===false?undefined:((()=>{current$Mnchapter=$Pl(current$Mnchapter,1);vertical$Mnscroll=0;return render();})()));}));
